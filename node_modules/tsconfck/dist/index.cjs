...
async function findAll(dir, options) {
  const files = [];
  for await (const tsconfigFile of findTSConfig(import_path2.default.resolve(dir), options)) {
...
async function* findTSConfig(dir, options, visited = /* @__PURE__ */ new Set()) {
  if (!visited.has(dir)) {
    visited.add(dir);
    try {
      const dirents = await import_fs2.promises.readdir(dir, { withFileTypes: true });
      for (const dirent of dirents) {
        if (dirent.isDirectory() && (!(options == null ? void 0 : options.skip) || !options.skip(dirent.name))) {
          yield* findTSConfig(import_path2.default.resolve(dir, dirent.name), options, visited);
        } else if (dirent.isFile() && dirent.name === "tsconfig.json") {
          yield import_path2.default.resolve(dir, dirent.name);
        }
      }
    } catch (e) {
